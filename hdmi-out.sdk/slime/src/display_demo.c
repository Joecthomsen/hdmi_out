...
void DemoRun()
{
	...
	while (userInput != 'q')
	{
		...
		case '6':
			...
			break;
		case '7':
			DemoGoatTest(pFrames[dispCtrl.curFrame], dispCtrl.vMode.width, dispCtrl.vMode.height, DEMO_STRIDE);
			break;
...
void DemoCRMenu()// Change resolution menu
{
	xil_printf("\x1B[H"); //Set cursor to top left of terminal
	xil_printf("\x1B[2J"); //Clear terminal
	xil_printf("**************************************************\n\r");
...
void DemoGoatTest(u8 *frame, u32 width, u32 height, u32 stride){
	...
	enum state {RED_UP , RED_DOWN, BLUE_UP , BLUE_DOWN, GREEN_UP , GREEN_DOWN};
	int state = RED_UP;

	u8 wRed = (fRed >= 256.0) ? 255 : ((u8) fRed);
	u8 wBlue = (fBlue >= 256.0) ? 255 : ((u8) fBlue);
	u8 wGreen = (fGreen >= 256.0) ? 255 : ((u8) fGreen);
...
void moveSquare(u8 *frame, u32 width, u32 height, u32 stride){
	...
	while(1)
	{
		...
		case 'd' :	// Move right   ///////   TODO FIX
			...
			//Draw new
			startPixelAddr += ((width/8) - 16) - (20*stride);
			wRed = 254;
			for(int i = 0 ; i < 20 ; i++)
			{
				for(int j = 0 ; j < 16  ; j+=4)
